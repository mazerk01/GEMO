...
export class HttpErrorInterceptor implements HttpInterceptor {
  ...
  // I toasts sono disattivati in mobile (window.screen.width > 767)

  intercept(req: HttpRequest<any>, next: HttpHandler): Observable<HttpEvent<any>> {
    return next.handle(req).pipe(
      map(res => {
        // Visualizza i Toastr di successo
        if (req.method == 'POST' && res.type == 4 && window.screen.width > 767) {
        ...
        if (req.method == 'PUT' && res.type == 4 && window.screen.width > 767) {
          this.toastr.success(`Articolo modificato`)
        }
        // check per delete prodotto/barcode
        if (req.method == 'DELETE' && res.type == 4 && window.screen.width > 767) {
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.status == 403 && window.screen.width > 767) this.toastr.error('Autorizzazioni insufficienti')
        // Login - credenziali mancanti
        if (error.status == 400 && error.message.includes('Login') && window.screen.width > 767) this.toastr.warning('Inserire Credenziali')
        // Login - Credenziali errate
        if (error.status == 404 && error.message.includes('Login') && window.screen.width > 767) this.toastr.error('Credenziali errate')
        // POST Prodotto: dati duplicati o mancanti
        if (error.status == 500 && window.screen.width > 767) {
        ...
        // Modifica
        if (req.method == 'PUT' && window.screen.width > 767) this.toastr.error('Modifica fallita')
        // Eliminazione fallita
        if (req.method == 'DELETE' && window.screen.width > 767) this.toastr.error('Eliminazione fallita')
